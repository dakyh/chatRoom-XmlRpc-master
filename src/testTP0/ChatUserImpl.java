...
public class ChatUserImpl {
	...
	public void connexion() {
		try {
			this.server = new XmlRpcClient("http://localhost:8085/RPC2");
	...
	public void requestPseudo() {
		this.pseudo = JOptionPane.showInputDialog(this.window, "Entrez votre pseudo : ", this.title, JOptionPane.OK_OPTION);
		if (this.pseudo == null)
			System.exit(0);
		...
		try {
			...
			if (!inscrireOk) {
				JOptionPane.showMessageDialog(this.window, "Pseudo existe déjà! Veuillez choisir un autre");
				requestPseudo();
			}
